...
class GraphletSampling(Kernel):
    ...
    def plot_all_graphlets(self, node_size = 5):
        ...
        for key, graphlet in self._networkx_graph_bins.items():  
...
def convert_to_networkx(graph_str):
        graph = nx.Graph()
        nodes = graph_str.strip("<>").split(")(")
        nodes = [node.strip("()") for node in nodes]
        for node in nodes:
            node = node.strip("]")
            node_id = int(node.split(" [")[0].split(" ")[0])
            neighbors = [int(n) for n in node.split(" [")[1].split(",")]
